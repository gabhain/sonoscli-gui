...
func NewSMAPIClient(ctx context.Context, speaker *Client, svc MusicServiceDescriptor, store SMAPITokenStore) (*SMAPIClient, error) {
	...
	// Clean deviceID to be alphanumeric only (e.g. remove colons from MAC or "uuid:RINCON_" prefix if present)
...
func (c *SMAPIClient) CompleteAuthentication(ctx context.Context, linkCode, linkDeviceID string) (SMAPITokenPair, error) {
	...
	if err := c.TokenStore.Save(c.Service.ID, c.HouseholdID, pair); err != nil {
		return SMAPITokenPair{}, fmt.Errorf("failed to save tokens for service %s: %w", c.Service.ID, err)
	}
	return pair, nil
}
...
func fetchPresentationMapURIFromManifest(ctx context.Context, httpClient *http.Client, manifestURI string) (string, error) {
	...
	req.Header.Set("User-Agent", "Sonos/34.7-22221 (V1)")
...
func (c *SMAPIClient) smapiCall(ctx context.Context, method string, args map[string]string, opts smapiCallOptions) ([]byte, error) {
	...
	req.Header.Set("X-Sonos-Household-Id", c.HouseholdID)
	...
	if resp.StatusCode == 500 {
		...
		if strings.Contains(fault.FaultCode, "Client.TokenRefreshRequired") {
			...
			req2.Header.Set("User-Agent", "Sonos/34.7-22221 (V1)")
			req2.Header.Set("Accept-Language", "en-US")
			req2.Header.Set("X-Sonos-Household-Id", c.HouseholdID)
...
func (c *SMAPIClient) buildCredentialsHeader(allowUnauthed bool) (string, error) {
	...
	creds.WriteString(`<appId>sonos</appId>`)

	switch c.Service.Auth {
	case MusicServiceAuthDeviceLink, MusicServiceAuthAppLink:
		creds.WriteString(`<context>sonos</context>`)
		if !allowUnauthed {
			...
			if !ok {
				return "", fmt.Errorf("service not authenticated: run auth for service ID %q and household ID %q", c.Service.ID, c.HouseholdID)
			}
...
func cleanDeviceID(id string) string {
	...
	// If it's a UDN like uuid:RINCON_000E58XXXXXX01400, extract the middle part.
	if strings.Contains(id, "RINCON_") {
		parts := strings.Split(id, "_")
		if len(parts) > 1 {
			id = parts[1]
		}
		// Some UDNs end with the port/suffix, e.g. 000E58XXXXXX01400
		if len(id) > 12 {
			id = id[:12]
		}
	}
	// If it's a serial like 00-0E-58-XX-XX-XX:X, remove the suffix.
