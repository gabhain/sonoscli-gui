...
func (sa *SonosApp) discoverIP(ip string) {
	...
	sa.Topology = top
	slog.Info("manual connection finished", "groups", len(top.Groups), "rooms", len(top.ByName))
	fyne.Do(func() {
...
func (sa *SonosApp) discover() {
	...
	sa.Topology = top
	slog.Info("discovery finished", "groups", len(top.Groups), "rooms", len(top.ByName))
...
func (sa *SonosApp) createRoomUI(name, ip, id string) *RoomUI {
	...
	ui.AudioSettings = container.NewVBox(
		...
		ui.LineOutCard,
		layout.NewSpacer(),
		makeCard("Modes", container.NewHBox(ui.LoudnessBtn, ui.NightBtn, ui.SpeechBtn)),
	)
	...
	ui.Tabs = container.NewAppTabs(
		container.NewTabItemWithIcon("Now Playing", theme.MediaPlayIcon(), container.NewCenter(nowPlaying)),
...
func (sa *SonosApp) updateInputsUI(ui *RoomUI, ip string) {
	...
	if err != nil {
		...
		// Don't return, we still want to check for TV input
	}

	items, _ := sonos.ParseDIDLItems(res.Result)
	
	// Add TV Input if it's a home theater device (simplified check: use self UUID from topology)
	var myUUID string
